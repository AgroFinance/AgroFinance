...
export default function OnboardingTour() {
  ...
  if (!isOpen) {
    return (
      <motion.button
        ...
        onClick={startTour}
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-4.5 py-3 rounded-full shadow-[0_8px_30px_rgb(19,124,83,0.25)] border border-[rgba(90,190,145,0.3)] bg-gradient-to-r from-[#2BA470] to-[#137C53] text-[#FBF4D6] font-semibold text-xs transition-all duration-300"
      >
  ...
  if (isMinimized) {
    return (
      <motion.div
        layout
        className="fixed bottom-6 right-6 z-50 glass-card p-3 rounded-2xl flex items-center gap-3 border border-[rgba(90,190,145,0.2)] shadow-xl cursor-pointer"
  ...
  const positionClasses = isCenter
    ...
    ? 'fixed bottom-6 left-6 max-w-sm w-full'
