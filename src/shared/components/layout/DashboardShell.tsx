...
function Brand({ light = false }: { light?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-2.5">
      <span className="w-8 h-8 rounded bg-white flex items-center justify-center flex-shrink-0 border border-[rgba(0,0,0,0.08)] overflow-hidden">
        <img src={`${BP}/logo.png`} alt="AgroFinance" className="w-6 h-6 object-contain" />
      </span>
      <span className="leading-tight">
        <span className={`block font-bold text-[14px] ${light ? 'text-white' : 'text-[#13301F]'}`}>AgroFinance</span>
        <span className={`block text-[9px] font-semibold tracking-[0.14em] ${light ? 'text-white/60' : 'text-[#5A6C5D]'}`}>CARBON INTELLIGENCE</span>
      </span>
    </Link>
  )
}
    ...
    <div className="flex flex-col h-full">
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-2.5 px-3 py-2 rounded text-[13px] font-medium transition-colors ${
                active
                  ? 'bg-white/12 text-white font-semibold border-l-2 border-[#16A864] pl-[10px]'
                  : 'text-white/75 hover:text-white hover:bg-white/[0.06] border-l-2 border-transparent pl-[10px]'
              }`}
            >
              <item.icon className="w-4 h-4 flex-shrink-0" />
              <span className="flex-1">{item.label}</span>
              {item.badge && (
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#16A864] text-white tracking-wide">
                  {item.badge}
                </span>
              )}
            </Link>
      ...
      <div className="p-3">
        <div className="rounded border border-white/10 p-2.5">
          <div className="text-[9px] font-semibold uppercase tracking-wider text-white/40">Campaña activa</div>
          <div className="text-[13px] font-semibold text-white mt-0.5">{EMPRESA.campania}</div>
        </div>
      </div>
    </div>
  )
        ...
        return (
          <Link
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-semibold transition-colors ${
              active
                ? 'bg-[#F4F6F2] text-[#137C53] border border-[#DDE3DE]'
                : 'text-[#5A6C5D] hover:text-[#13301F] hover:bg-[#F4F6F2]'
            }`}
          >
...
export default function DashboardShell({ children, onExport }: DashboardShellProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F2]">
      {/* Sidebar fijo (desktop) — color solido, no gradiente: estilo
          software empresarial clasico (SAP/QuickBooks) en vez de "startup". */}
      <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-60 z-40 bg-[#0F3D2C]">
        ...
        {drawerOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 w-[78%] max-w-[280px] z-[56] lg:hidden bg-[#0F3D2C]"
            >
      ...
      <div className="lg:pl-60">
        {/* Top bar */}
        <header className="sticky top-0 z-30 h-14 bg-white border-b border-[#DDE3DE]">
          <div className="h-full px-4 sm:px-6 flex items-center gap-3">
            <button
              onClick={() => setDrawerOpen(true)}
              className="lg:hidden w-9 h-9 -ml-1 rounded border border-[#DDE3DE] flex items-center justify-center text-[#137C53]"
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-semibold text-red-700 hover:bg-red-50 border border-[#DDE3DE] hover:border-red-300 transition-colors"
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded bg-[#137C53] flex items-center justify-center text-white text-xs font-bold flex-shrink-0 select-none" title={`${user.nombre} · ${user.empresa}`}>
                  {user.avatarInitials}
                </div>
                <button
                  onClick={() => { logout(); router.replace('/') }}
                  title="Cerrar sesión"
                  className="w-8 h-8 rounded border border-[#DDE3DE] flex items-center justify-center text-[#5A6C5D] hover:text-red-600 hover:border-red-300 transition-colors"
                >
              ...
              <Link
                href="/login/"
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded border border-[#DDE3DE] text-xs font-semibold text-[#137C53] hover:bg-[#F4F6F2] transition-colors whitespace-nowrap"
              >
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded bg-[#13301F] text-white text-xs font-semibold hover:bg-[#0E2418] transition-colors whitespace-nowrap"
            >
        ...
        {showClearConfirm && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded w-full max-w-sm overflow-hidden border border-[#DDE3DE]"
            >
              <div className="p-6 text-center">
                ...
                <p className="text-sm text-[#5A6C5D] mb-6">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 rounded border border-[#DDE3DE] text-sm font-semibold text-[#13301F] hover:bg-[#F4F6F2] transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 rounded bg-red-600 text-white text-sm font-semibold hover:bg-red-700 transition-colors"
                  >
                    Limpiar Datos
                  </button>
                </div>
