    ...
    <DashboardShell>
      <div className="relative max-w-4xl mx-auto pb-16">
        ...
        <AnimatePresence mode="wait">
          ...
          {stage === 'uploading' && (
            // Minimalista a pedido: una fila por archivo (icono + nombre +
            // barra fina + %), no la tarjeta grande centrada de antes. Con
            // lote de más de uno, los ya completados quedan arriba como
            // referencia — así "archivo por archivo" se ve como una lista
            // que avanza, no como una sola pantalla que se resetea por
            // cada archivo.
            <motion.div key="uploading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="bg-white rounded border border-slate-200 p-5 max-w-md mx-auto">
              {registrados.map((p) => (
                <div key={p.huella} className="flex items-center gap-3 py-2 border-b border-slate-100 last:border-0">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0" />
                  <span className="text-sm text-slate-500 truncate">{p.fileName}</span>
                  <span className="text-xs text-slate-400 ml-auto flex-shrink-0">Listo</span>
                </div>
              ))}
              <div className="flex items-center gap-3 py-2">
                ...
                <span className="text-sm font-medium text-slate-800 truncate flex-1">{files[0]?.name}</span>
                <span className="text-xs font-semibold text-emerald-600 flex-shrink-0">{progress}%</span>
              </div>
