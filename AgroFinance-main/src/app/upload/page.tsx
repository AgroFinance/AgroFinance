...
import DepuracionLineas from '@/components/ui/DepuracionLineas';
import Link from 'next/link';
import { generateExecutivePdfReport } from '@/lib/reports/pdfGenerator';
import { parsearUBL, huellaArchivo, type CabeceraUBL } from '@/lib/parsing/parseArchivo';
import { ghgClassify, resumirLineas, type LineaClasificada, type ResumenClasificacion } from '@/lib/engine/ghgClassify';
import { useFuentesDatos, type FuenteDatos } from '@/lib/store/datosPrueba';
import { useSesionUpload } from '@/lib/store/useSesionUpload';
import { MECANISMO_META, type Mecanismo } from '@/lib/engine/emissionFactors';
...
interface ResultadoCarga {
  ...
  /**
   * Resumen ya calculado por la Cloud Function sobre el archivo COMPLETO
   * (no solo el preview de `lineas`, que la nube capa a 400 filas para
   * no reventar el documento de Firestore). Cuando está presente se usa
   * tal cual — recalcular con resumirLineas(lineas) subestimaría el
   * total en archivos grandes. La demo de 1 clic no lo trae: ahí sí las
   * `lineas` están completas y resumirLineas es exacto.
   */
  resumenServidor?: ResumenClasificacion;
}

// El comprobante de demostración trae, a propósito, dos líneas de consumo
// energético y dos líneas que NO son consumo (un servicio y un alquiler):
// así se ve que la plataforma descarta lo que no corresponde en vez de
// meterlo al total, que es justo lo que revisa un auditor.
...
export default function UploadPage() {
  ...
  // ------------------------------------------------------------
  // Registro automático como fuente de datos (aparece en Configuración
  // sin que nadie tenga que volver a subir el archivo) y actualización del
  // store compartido cada vez que se corrige un mapeo.
  // ------------------------------------------------------------
  useEffect(() => {
  ...
  // Demo de 1 clic: sigue 100% síncrona en el navegador (motor cliente
  // real, mismo parsearUBL/ghgClassify de siempre) — a propósito, para
  // tener una demo rápida sin depender de Storage/Firestore/Function.
  // No usa sesion.subir(): es el único camino que NO pasa por la cola.
  const procesarDemo = () => {
    setStage('uploading');
    setProgress(0);
    setErrorMsg('');
  ...
  // Carrusel cosmético de mensajes mientras la Cloud Function procesa de
  // verdad en la nube — el progreso no se inventa por pasos fijos como
  // antes: avanza con el tiempo real transcurrido y se detiene en 95%
  // hasta que la sesión pase a 'completado' (o 'error').
  ...
  // El carrusel cosmético del caso real (sin `r`) no tiene un punto de
  // parada propio — lo detiene este efecto en cuanto la sesión deja de
  // estar 'procesando', y también al desmontar la pantalla.
  useEffect(() => {
  ...
  // Archivo real soltado por el usuario: sube a Storage, crea la sesión
  // 'pendiente' y espera a que la Cloud Function la procese — ver
  // lib/store/useSesionUpload.ts. El efecto de abajo traduce el estado
  // de esa sesión a `stage`/`resultado` de esta pantalla.
  useEffect(() => {
    ...
    if (sesion.estado === 'procesando') {
      if (stage !== 'scanning') return iniciarEscaneo(); // arranca el carrusel una sola vez
      return;
    }
        ...
        <AnimatePresence mode="wait">
          ...
          {stage === 'complete' && resultado && resumen && (
            <motion.div key="complete" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="space-y-8">
              ...
              {/* Registro automático como fuente */}
              ...
              <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
                ...
                {/* Depuración: resumen visible, detalle bajo demanda */}
                <DepuracionLineas
