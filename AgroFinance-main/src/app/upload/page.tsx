...
import { parsearUBL, parsearArchivo, huellaArchivo, type CabeceraUBL } from '@/lib/parseArchivo';
import { ghgClassify, resumirLineas, type LineaClasificada } from '@/lib/ghgClassify';
...
export default function UploadPage() {
  ...
  const resumen = useMemo(() => (resultado ? resumirLineas(resultado.lineas) : null), [resultado]);
  ...
  const procesarArchivo = (file: File | null) => {
    ...
    const uploadInterval = setInterval(() => {
      ...
      const leer = async (): Promise<ResultadoCarga> => {
        if (!file) {
          const r = await parsearUBL(sampleXmlContent, 'Factura_SUNAT_UBL21_Demo.xml');
          return {
        ...
        if (file.name.toLowerCase().endsWith('.xml')) {
          const r = await parsearUBL(await file.text(), file.name);
          return {
            fileName: file.name, huella: huellaArchivo(file), cabecera: r.cabecera,
            lineas: ghgClassify(r.lineas), hojas: r.hojas, columnas: r.columnas, filasPreview: r.filasPreview,
          };
        }
        // Un .xlsx/.csv también entra por aquí: mismo módulo de clasificación.
        const r = await parsearArchivo(file);
        return {
          fileName: file.name, huella: huellaArchivo(file),
          cabecera: { invoiceId: file.name, ruc: '—', proveedor: '—', fecha: '—', moneda: 'PEN', montoTotal: null },
          lineas: ghgClassify(r.lineas), hojas: r.hojas, columnas: r.columnas, filasPreview: r.filasPreview,
        };
      };

      leer()
        .then((r) => iniciarEscaneo(r))
        .catch((e) => {
