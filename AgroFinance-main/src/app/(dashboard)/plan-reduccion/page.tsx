...
import {
  Sparkles, Download, FileSpreadsheet, RotateCcw, CheckCircle2, Lock, AlertTriangle,
} from 'lucide-react'
...
} from '@/modules/green-financing/domain/reduccionActions'
...
export default function PlanReduccionPage() {
  ...
  const totalTon = huella.huellaTotalTon
  const acciones = useMemo(() => construirAcciones(huella), [huella])
  const metaTon = totalTon * (METALL.pctObjetivo / 100)

  const [seleccionadas, setSeleccionadas] = useState<Set<string>>(new Set())

  const acumuladoTon = useMemo(
    () => acciones.filter((a) => seleccionadas.has(a.id)).reduce((sum, a) => sum + reduccionTon(a, huella), 0),
    [acciones, seleccionadas, huella],
  )
  const progresoPct = metaTon > 0 ? Math.min(100, (acumuladoTon / metaTon) * 100) : 0
  const metaCumplida = totalTon > 0 && acumuladoTon >= metaTon
  ...
  const queKapiArmeElPlan = () => setSeleccionadas(armarPlanKapi(acciones, huella))
  ...
  const exportarCSV = () => {
    ...
    const rows = acciones.map((a) => [
      a.titulo, a.categoria, `Alcance ${a.scope}`, fmtTon(reduccionTon(a, huella)),
      fmtPct(reduccionPct(a, huella)), a.inversionLabel, seleccionadas.has(a.id) ? 'Sí' : 'No',
    ])
  ...
  return (
    <DashboardShell>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        {!huella.tieneDatos && (
          <div className="rounded-2xl border border-[rgba(210,145,47,0.3)] bg-[rgba(210,145,47,0.06)] p-4 mb-6 flex items-start gap-3 print:hidden">
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-[#13301F]">Sin datos vinculados</h3>
              <p className="text-xs text-[rgba(80,108,92,0.75)] leading-relaxed mt-0.5">
                No hay fuentes de datos activas, así que no hay huella sobre la cual calcular reducciones.
                Vincula tus archivos o restaura los datos demo en{' '}
                <Link href="/configuracion/" className="underline font-semibold hover:text-[#13301F]">Configuración</Link>.
              </p>
            </div>
          </div>
        )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {acciones.map((a) => {
                ...
                const pct = reduccionPct(a, huella)
